...
export default function Student({isLoading, user}: StudentProps) {
  ...
  <div className=" h-400 overflow-hidden ">
    ...
    <div className="w-180 mt-5 ml-110">
      ...
      <div>
            <SidebarProvider>
              <Sidebar className=" min-h-screen absolute mt-32 ml-2 w-100 rounded-4xl h-300  border-r  text-slate-900 font-sans  flex-col">
                  ...
                  <div className="space-y-1">
                        <Button onClick={() => handleSidebar("dashboard")}
                          ...
                          <Button  onClick={() => handleSidebar("profile")}
                              className={`${dash === "dashboard" ? "bg-white text-black" : "bg-black text-white"} px-3 py-2 rounded-lg text-sm font-medium transition-all `}>
